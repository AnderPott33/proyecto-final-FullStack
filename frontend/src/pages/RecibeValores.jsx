...
export default function RecibeValores() {
  ...
  const handleRegistrarMovimiento = async () => {
    ...
    if (loading) return
    setLoading(true); // Solo activar si pasó la validación
    ...
    try {
      ...
      const payload = {
        ...
        descripcion: descripcionInput.value.trim(), // asegurarse de enviar sin espacios
  ...
  // TOTALES
  const totalDebito = itemsDetalle.filter(i => i.tipo === "CRÉDITO").reduce((acc, i) => acc + i.monto, 0);
  const totalCredito = itemsDetalle.filter(i => i.tipo === "DÉBITO").reduce((acc, i) => acc + i.monto, 0);
    ...
    <>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6
          ...
          <div>
            ...
            <p className="text-white/90 text-sm md:text-base mt-2">
              Realice todas las entradas de la empresa.
            </p>
          </div>
      ...
      <div className="grid md:grid-cols-1 gap-2">
        <div className="bg-white rounded-md shadow-md border border-[#35b9ac] py-2 px-1 overflow-x-auto">
          <div className="flex justify-between flex-wrap">
            <h2 className="font-semibold text-2xl mt-2 ml-4 text-[#35b9ac] mb-3">ENTRADAS</h2>
            <button onClick={() => setOpenModalBaja(true)} className="p-2 m-2 bg-[#35b9ac] hover:bg-[#35b9ac]/75 cursor-pointer outline-none text-white py-3 rounded-md flex items-center justify-center gap-2 font-semibold shadow"><FaMinusSquare /> Agregar Entrada</button>
          </div>
          <DataTable
            ...
            showPDFButton={false}
          />
          <div className="text-right mt-3 mr-2 font-bold text-[#35b9ac]">{formatearNumero(totalDebito, moneda)}</div>
        </div>

        {/* FORMAS DE COBRO */}
        <div className="bg-white rounded-md shadow-md border border-[#35b9ac] py-2 px-1 overflow-x-auto">
          <div className="flex justify-between flex-wrap">
            <h2 className="font-semibold text-2xl mt-2 ml-4 text-[#35b9ac] mb-3">FORMAS DE COBRO</h2>
            <button onClick={() => setOpenModal(true)} className="p-2 m-2 bg-[#35b9ac] hover:bg-[#35b9ac]/75 cursor-pointer outline-none text-white py-3 rounded-md flex items-center justify-center gap-2 font-semibold shadow"><FaPlusSquare /> Agregar Forma de Cobro</button>
          </div>
          <DataTable
      ...
      < div className="mt-2 bg-white rounded-md mb-3 shadow-md border border-[#359bac] p-2 flex justify-between items-center" >
        <div className="bg-red-200 p-2 rounded-xl">
          <p className="text-white text-shadow-lg shadow-black font-bold">Entradas</p>
          <p className="text-xl font-bold text-red-600">{formatearNumero(totalDebito, moneda)}</p>
        </div>
        <div className="bg-green-200 p-2 rounded-xl">
          <p className="text-white text-shadow-lg shadow-black font-bold">Cobros</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white w-full max-w-[800px] max-h-[650px] rounded-md shadow-xl p-6">
              <div className="flex justify-between items-center mb-2">
                <h2 className="text-lg font-semibold text-gray-800">Agregar Cobros</h2>
      ...
      {/* AGREGAR DÉBITO */}
      {
        openModalBaja && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white w-full max-w-[800px] max-h-[650px] rounded-md shadow-xl p-6">
              <div className="flex justify-between items-center mb-2">
                <h2 className="text-lg font-semibold text-gray-800">Agregar Entradas</h2>
